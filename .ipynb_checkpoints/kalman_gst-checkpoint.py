...
class ExtendedKalmanFilter():
    ...
    param_history: a list of the history of the parameter estimates
    covar_history: a list of the history of the covariance estimates
    """
    ...
    def update(self, circ, count_vec, clip_range=[-1,1], Q=None, R_additional=None):
        """
    ...
    def update_approx(self, circ, count_vec, p0, jac0, hess0, clip_range=[-1, 1], Q=None, R_additional=None):
        """
        Makes an approximate update to the model
        where the jacobian is approximated
        ...
        NOTE: the difference between this *update_approx* and *update_fast* is that 
            *approx also approximates the model prediction using the hessian and jacobian, 
            which is even faster than fast but even less accurate (works decent with only stochastic noise)
        """
        prior_covar = self.P
        prior_state = self.model.to_vector()
        hilbert_dims = 2**(circ.width)
    ...
    def filter_dataset(self, circuit_list, dataset):
        """
        Batch filter of the given circuits using exact updates
        """
    ...
    def fast_filter_dataset(self, circuit_list, dataset, jacobians, hessians):
        """
        batch filter of the given circuits using fast updates
        """
        for circ in tqdm(circuit_list):
            count_vec = vector_from_outcomes(dataset[circ].counts, 2**circ.width)
