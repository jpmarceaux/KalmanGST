...
max_stochastic_error_rate_2qb = 0.001

hamiltonian_error_var_1qb = 0.005
...
maxLengths = [1, 2, 4]
fpr_list = find_sufficient_fiducial_pairs_per_germ_greedy(target_model, std.prep_fiducials(), std.meas_fiducials(), std.germs(), 
    inv_trace_tol=10, sensitivity_threshold=1e-5, evd_tol=1e-7, 
    check_complete_fid_set=False, initial_seed_mode='greedy', verbosity=1)

edesign_old = pygsti.protocols.StandardGSTDesign(target_model, std.prep_fiducials(), std.meas_fiducials(),
                                                std.germs(), maxLengths)
edesign = pygsti.protocols.StandardGSTDesign(target_model, std.prep_fiducials(), std.meas_fiducials(),
                                                std.germs(), maxLengths, fiducial_pairs=fpr_list)
...
#     axs[0].plot(ekf_error)
#     axs[1].plot(ekf_uncert)
...
#     plt.show()
#     fig.savefig('Figures/MSE_evolution_2qb.eps', dpi=350, format="eps", bbox_inches='tight')